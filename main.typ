...
#figure(
    image("figures/intro2.svg", height: 6em),
...
In the following sections, we define the physics that relate mass distribution to gravity vector field at the Earth's surface, propose an extension to the simple cylindrical model of Earth and an approach for numerical evaluation, and finally show solutions that meet gravimetry constraints.
...
By the identity theorem of harmonic functions, the requirement that $||g(x) - g_0|| = 0$ on the disk $D$ would require infinite mass in $ρ(x')$ (e.g. an infinite uniform slab). However, relaxing the constraint to allow deviations in an ε-ball around $g_0$ (as in @eball) permits solutions with finite mass.  Indeed, gravimetric measurements show variations in local gravity field on the surface of the Earth up to \~0.1% (ε=0.001) @anomaly7.

#figure(
    image("figures/physics2.svg"),
    caption: [Approximately uniform gravity field on a disk with error up to ε]
) <eball>
...
#figure(
    ...
    caption: [Example ASS model mass distributions and associated $b(r')$ profiles.  These distributions are not necessarily minimum mass nor do they satisfy the ε constraint.]
) <assmodel>
...
$
    g(x, x') = G rho(x') / (||x - x'||^2) dot.op (x' - x) / (||x - x'||) = rho(x') dot.op (x' - x) / (||x - x'||^3)
$
